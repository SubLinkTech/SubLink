...
. dotnet.exe publish -c Release -r linux-x64 $currentDir/SubLink.References/SubLink.References.csproj /p:Version=$version /p:SkipInvalidConfigurations=true;
. dotnet.exe publish -c Release -r linux-x64 $currentDir/SubLink/SubLink.csproj /p:Version=$version /p:SkipInvalidConfigurations=true /p:PublishSingleFile=true /p:PublishReadyToRun=true /p:IncludeNativeLibrariesForSelfExtract=true /p:EnableCompressionInSingleFile=true --self-contained true
. dotnet.exe publish -c Release -r linux-x64 $currentDir/SubLink.Twitch/SubLink.Twitch.csproj /p:Version=$version /p:SkipInvalidConfigurations=true
...
Copy-Item -Path "SubLink.Twitch\bin\Release\net8.0\linux-x64\publish\SubLink.Twitch.dll" -Destination "build-$($version)\Platforms";
...
if (Test-Path builds\SubLink-$version-linux-x64.zip) {
    Remove-Item builds\SubLink-$version-linux-x64.zip;
}

Compress-Archive -Path "build-$version\*" -destinationpath "builds\SubLink-$version-linux-x64.zip" -compressionlevel optimal;
